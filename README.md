...
> [!CAUTION]
> Автоматического перехеширования в хеш-таблице нет, потому что это долгая операция, которая может приводить к серьезным простоям. Если вам оно необходимо, то можете скомпилировать программу с флагом -DRESIZABLE_USER=ON и написать свой rehashUser в файле include/rehash_user.h. Обязательно используйте флаг -DLOAD_FACTOR=<your_load_factor>, иначе он будет определен в 0.
